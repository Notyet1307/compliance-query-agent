...
func (e *Engine) Query(ctx context.Context, r Request) (*Result, bool, error) {
	...
	defer cancel()
	result, runErr := e.execute(ctx, r, input)
	if err = e.store.finish(r.RequestID, input, e.configDigest, result, runErr); err != nil {
		return nil, false, err
	}
	return result, false, runErr
}
func (e *Engine) execute(ctx context.Context, r Request, input string) (*Result, error) {
	...
	claims, err := e.generator.Generate(ctx, r, selected.sources)
	if err != nil {
		return nil, err
	}
