...
result={'status':'PASS' if not failed else 'FAIL','go_test_roots':len([n for n in passed if '/' not in n]),'go_test_events_including_subtests':len(passed),'smoke':json.loads((p/'smoke.json').read_text()),'evidence_directory':str(p)}
